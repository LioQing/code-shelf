...
unknown index, we need to use a temporary array to store the
...
def zeroOneKnapsack(capacity: int, items: list[Item]):
    ...
    for i in range(1, n + 1):
        ...
        for w in range(1, capacity + 1):
            if w - items[i - 1].weight >= 0:
                dp[w] = max(
                    prev_dp[w - items[i - 1].weight] + items[i - 1].value,
                    prev_dp[w],
                )
            else:
                dp[w] = prev_dp[w]
...
float zero_one_knapsack(int capacity, std::vector<Item> items) {
    ...
    // initialize memoization array
    std::vector<float> dp(capacity + 1, 0);

    // calculate values
    for (int i = 1; i <= n; i++) {
        std::vector<float> prev_dp = dp;
